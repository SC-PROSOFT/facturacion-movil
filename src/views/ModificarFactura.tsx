...
/* utils */
import {formatToMoney, redefineBills} from '../utils';
...
const ModificarFactura: React.FC = () => {
  ...
  const toggleSaveInvoice = async () => {
    ...
    try {
      ...
      const factura = await estructurarFactura();
      await facturasApiService._saveFactura(factura, 'put');
      await facturasService.updateFactura(objOperador.nro_factura.toString(), {
        ...factura,
        sincronizado: 'S',
      });
      const redefinedBills = redefineBills(arrFacturas, factura);
      dispatch(setArrFactura(redefinedBills));
  ...
  const saveInvoiceInLocaldatabaseOnly = async (): Promise<void> => {
    try {
      ...
      dispatch(
        setObjOperator({
          ...factura.operador,
          nro_factura: Number(factura.operador.nro_factura),
        }),
      );
      resetState();
      Toast.show({
  ...
  const estructurarFactura = async (): Promise<IOperation> => {
    ...
    return {
      ...
      operador: {
        ...objOperador,
        nro_factura: Number(objOperador.nro_factura),
      },
      tercero: objTercero,
    ...
    <View>
      ...
      <ScrollView
        ...
        <View style={{marginHorizontal: 10, marginTop: 10, paddingBottom: 120}}>
          <CoolButton
            value={isLoadingSave ? '' : 'Modificar Factura'}
