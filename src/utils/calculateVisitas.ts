...
const FixedDayRule: ZoneRule = {
  matches: (zoneCode, date) => {
    const dayMapping: Record<string, DayOfWeek> = {
      '01': DayOfWeek.Lunes,
      '02': DayOfWeek.Martes,
      '03': DayOfWeek.Miercoles,
      '04': DayOfWeek.Jueves,
      '05': DayOfWeek.Viernes,
      '06': DayOfWeek.Sabado,
...
const NthWeekdayRule: ZoneRule = {
  matches: (zoneCode, date) => {
    ...
    return (
      date.getDay() === dayOfWeekInCode &&
      getPreciseWeekdayOccurrence(date) === occurrence
    );
  },
};
...
const zoneSchedulingRules: ZoneRule[] = [
  FixedDayRule,
  NthWeekdayRule,
  DualDayRule,
];
...
async function shouldRecalculateVisits(): Promise<boolean> {
  try {
    ...
    console.log(
      'Visitas: Última fecha de generación:',
      lastDate,
      'Hoy:',
      today,
    );
    ...
    console.error(
      'Visitas: Error verificando última fecha de generación:',
      e.message,
    );
...
function getTerceroFrequencies(tercero: ITerceros): string[] {
  const freqs: (string | undefined)[] = [
    tercero.frecuencia,
    (tercero as any).frecuencia_1, // Mantener alias por retrocompatibilidad
    tercero.frecuencia2,
    (tercero as any).frecuencia_2,
    tercero.frecuencia3,
    (tercero as any).frecuencia_3,
  ];
...
export async function generatePotentialVisits(
  ...
  console.log(
    ...
    datesToCheck.map(d => d.dateStr).join(', '),
  );

  try {
    const tercerosToProcess =
      allTerceros || (await tercerosService.getAllTerceros());
    ...
    const existingVisitsToday = await visitaService.getVisitasByDate(
      datesToCheck[0].dateStr,
    );
    const existingVisitsTomorrow = await visitaService.getVisitasByDate(
      datesToCheck[1].dateStr,
    );
    ...
    console.log(
      `Visitas: Encontradas ${existingVisitKeys.size} visitas existentes para hoy y mañana.`,
    );
      ...
      datesToCheck.forEach(({dateObj, dateStr}) => {
        ...
        if (isScheduledForThisDate) {
          ...
          const alreadyExistsInCurrentBatch = potentialVisits.some(
            v =>
              v.id_tercero === tercero.codigo && v.appointmentDate === dateStr,
          );
...
export async function recalculateAndSaveVisitsIfNeeded(): Promise<IVisita[]> {
  try {
    ...
    if (shouldRecalculate) {
      ...
      // 1. Eliminar visitas pendientes futuras

      // 2. Generar nuevas visitas potenciales
      const newPotentialVisits = await generatePotentialVisits();

      if (newPotentialVisits.length > 0) {
        // 3. Guardar todas las visitas nuevas en una sola operación masiva
        console.log(
          `Visitas: Guardando ${newPotentialVisits.length} nuevas visitas en lote...`,
        );
        const savedVisits = await visitaService.createBulkVisitas(
          newPotentialVisits,
        );
        console.log(
          `Visitas: Se guardaron ${savedVisits.length} nuevas visitas en la BD.`,
        );
        return savedVisits;
      }
      return [];
    ...
    console.error(
      'Visitas: Error fatal durante el recalculo de visitas:',
      error.message,
    );
    return [];
  }
